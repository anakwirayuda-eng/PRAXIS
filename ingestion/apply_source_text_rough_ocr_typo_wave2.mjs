...
const REPORT_FILE = join(__dirname, 'output', 'source_text_rough_ocr_typo_wave2_report.json');

const WAVE2_RE = /\b(?:aery|aeries|aerial|aoic|(?:ascending|descending|abdominal|thoracic) aoa|veebral|veebrae|veebrate|veebroplasty|poion|coicle|fouh|ceain|paial|paially|staing)\b|interveebral/i;

const REPLACEMENTS = [
  [/\baeries\b/gi, preserveCase('arteries')],
  [/\baery\b/gi, preserveCase('artery')],
  [/\baerial\b/gi, preserveCase('arterial')],
  [/\baoic\b/g, 'aortic'],
  [/\b((?:ascending|descending|abdominal|thoracic) )aoa\b/gi, (_match, prefix) => `${prefix}aorta`],
  [/interveebral/gi, preserveCase('intervertebral')],
  [/\bveebroplasty\b/gi, preserveCase('vertebroplasty')],
  [/\bveebral\b/gi, preserveCase('vertebral')],
  [/\bveebrae\b/gi, preserveCase('vertebrae')],
  [/\bveebrate\b/gi, preserveCase('vertebrate')],
  [/\bpoion\b/gi, preserveCase('portion')],
  [/\bcoicle\b/gi, preserveCase('cortical')],
  [/\bfouh\b/gi, preserveCase('fourth')],
  [/\bceain\b/gi, preserveCase('certain')],
  [/\bpaially\b/gi, preserveCase('partially')],
  [/\bpaial\b/gi, preserveCase('partial')],
  [/\bstaing\b/gi, preserveCase('starting')],
];
...
function normalizeWave2(value) {
  if (typeof value !== 'string' || !WAVE2_RE.test(value)) {
    return value;
  }
...
function mutateCase(caseRecord) {
  ...
  for (const fieldPath of collectFieldPaths(caseRecord)) {
    ...
    const next = normalizeWave2(current);
    if (next !== current) {
  ...
  if (changes.length > 0) {
    ...
    caseRecord.meta._source_text_rough_ocr_typo_wave2 = {
      applied_at: new Date().toISOString(),
      changed_fields: changes.map((change) => change.field),
      rule: 'second-pass missing-letter OCR typo normalization',
    };
  }
...
function main() {
  ...
  console.log('Source text rough OCR typo wave2 applied');
