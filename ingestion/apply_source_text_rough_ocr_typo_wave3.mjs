...
const REPORT_FILE = join(__dirname, 'output', 'source_text_rough_ocr_typo_wave3_report.json');

const WAVE3_RE = /\b(?:cailage|fibrocailage|ahritis|ahroscopy|pubey|poal|poals|veical|firous|diapgragm|distoion|locaton|hyperophy|characterstic|intesity|suppoed|unsuppoed|repo|repos)\b/i;

const REPLACEMENTS = [
  [/\bfibrocailage\b/gi, preserveCase('fibrocartilage')],
  [/\bcailage\b/gi, preserveCase('cartilage')],
  [/\bahroscopy\b/gi, preserveCase('arthroscopy')],
  [/\bahritis\b/gi, preserveCase('arthritis')],
  [/\bpubey\b/gi, preserveCase('puberty')],
  [/\bpoals\b/gi, preserveCase('portals')],
  [/\bpoal\b/gi, preserveCase('portal')],
  [/\bveical\b/gi, preserveCase('vertical')],
  [/\bfirous\b/gi, preserveCase('fibrous')],
  [/\bdiapgragm\b/gi, preserveCase('diaphragm')],
  [/\bdistoion\b/gi, preserveCase('distortion')],
  [/\blocaton\b/gi, preserveCase('location')],
  [/\bhyperophy\b/gi, preserveCase('hypertrophy')],
  [/\bcharacterstic\b/gi, preserveCase('characteristic')],
  [/\bintesity\b/gi, preserveCase('intensity')],
  [/\bunsuppoed\b/gi, preserveCase('unsupported')],
  [/\bsuppoed\b/gi, preserveCase('supported')],
  [/\brepos\b/gi, preserveCase('reports')],
  [/\brepo\b/gi, preserveCase('report')],
];
...
function normalizeWave3(value) {
  if (typeof value !== 'string' || !WAVE3_RE.test(value)) {
    return value;
  }
...
function mutateCase(caseRecord) {
  ...
  for (const fieldPath of collectFieldPaths(caseRecord)) {
    ...
    const next = normalizeWave3(current);
    if (next !== current) {
  ...
  if (changes.length > 0) {
    ...
    caseRecord.meta._source_text_rough_ocr_typo_wave3 = {
      applied_at: new Date().toISOString(),
      changed_fields: changes.map((change) => change.field),
      rule: 'third-pass high-confidence medical OCR typo normalization',
    };
  }
...
function main() {
  ...
  console.log('Source text rough OCR typo wave3 applied');
