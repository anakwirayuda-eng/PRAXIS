...
const REPORT_FILE = join(__dirname, 'output', 'source_text_rough_ocr_typo_wave5_report.json');

const WAVE5_RE = /\b(?:hypeensive|antihypeensives?|shoer|shoest|shoness|megahase|ahropods?|paicles?|betablockers?|cardiomyopthy|pappilomas?|pappilary|pappilomatous|repoed(?:ly)?|mensus|Borrlia|abdomianl)\b|inveebrate|inveed|veigo|deoxyribonuc\]eosides|\bsta antibiotics\b/i;

const REPLACEMENTS = [
  [/\bHypeensive\b/g, 'Hypertensive'],
  [/\bhypeensive\b/g, 'hypertensive'],
  [/\babdomianl\b/gi, preserveCase('abdominal')],
  [/\bantihypeensives\b/gi, preserveCase('antihypertensives')],
  [/\bantihypeensive\b/gi, preserveCase('antihypertensive')],
  [/\bshoest\b/gi, preserveCase('shortest')],
  [/\bshoer\b/gi, preserveCase('shorter')],
  [/\bshoness\b/gi, preserveCase('shortness')],
  [/\bmegahase\b/gi, preserveCase('megabase')],
  [/\bahropods\b/gi, preserveCase('arthropods')],
  [/\bahropod\b/gi, preserveCase('arthropod')],
  [/inveebrate/gi, preserveCase('invertebrate')],
  [/\bpaicles\b|\bpaicles(?=[A-Z])/gi, preserveCase('particles')],
  [/\bpaicle\b|\bpaicle(?=[A-Z])/gi, preserveCase('particle')],
  [/\bbetablockers\b/gi, preserveCase('beta-blockers')],
  [/\bbetablocker\b/gi, preserveCase('beta-blocker')],
  [/\bcardiomyopthy\b/gi, preserveCase('cardiomyopathy')],
  [/\bpappilomas\b/gi, preserveCase('papillomas')],
  [/\bpappiloma\b/gi, preserveCase('papilloma')],
  [/\bpappilary\b/gi, preserveCase('papillary')],
  [/\bpappilomatous\b/gi, preserveCase('papillomatous')],
  [/\binveed\b|inveed(?=[A-Z])/gi, preserveCase('inverted')],
  [/\bveigo\b|veigo(?=[A-Z])/gi, preserveCase('vertigo')],
  [/\bdeoxyribonuc\]eosides\b/gi, preserveCase('deoxyribonucleosides')],
  [/\brepoedly\b/gi, preserveCase('reportedly')],
  [/\brepoed\b/gi, preserveCase('reported')],
  [/\bsta antibiotics\b/gi, preserveCase('start antibiotics')],
  [/\bBorrlia\b/g, 'Borrelia'],
  [/\bmensus\b/gi, preserveCase('menses')],
];
...
function collectFieldPaths(caseRecord) {
  ...
  if (caseRecord.meta && typeof caseRecord.meta === 'object') {
    paths.push('meta.answer_anchor_text', 'meta.ai_suggested_answer');
  }

  return paths;
}

function normalizeWave5(value) {
  if (typeof value !== 'string' || !WAVE5_RE.test(value)) {
    return value;
  }
...
function mutateCase(caseRecord) {
  ...
  for (const fieldPath of collectFieldPaths(caseRecord)) {
    ...
    const next = normalizeWave5(current);
    if (next !== current) {
  ...
  if (changes.length > 0) {
    ...
    caseRecord.meta._source_text_rough_ocr_typo_wave5 = {
      applied_at: new Date().toISOString(),
      changed_fields: changes.map((change) => change.field),
      rule: 'fifth-pass high-confidence OCR typo normalization',
    };
  }
...
function main() {
  ...
  console.log('Source text rough OCR typo wave5 applied');
